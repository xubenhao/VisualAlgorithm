...
#include "arrayorganizedrawingwidget.h"


ArrayOrganizeDrawingWidget::ArrayOrganizeDrawingWidget(QWidget *parent) : QWidget(parent)
{
    Initialize();
}

void ArrayOrganizeDrawingWidget::ResetAllParts()
{
    ...
    _nSize = m_arrEleAddrParts.GetSize();
    for(int i = 0; i < _nSize; i++)
    {
        m_arrParts.Add(m_arrEleAddrParts[i]);
    }
...
void ArrayOrganizeDrawingWidget::Initialize()
{
    ResetAllParts();
}

void ArrayOrganizeDrawingWidget::AdjustInitialTransform()
{
...
void ArrayOrganizeDrawingWidget::paintEvent(QPaintEvent* event)
{
    ...
    // For Debug
    QString _filepath = NNcManager::NcManager::GetGeneralArraySearchFile((char*)"organ_show_test");
    QFile _file;
    _file.close();
