...
#include "arrayvaluevisitdrawingwidget.h"

ArrayValueVisitDrawingWidget::ArrayValueVisitDrawingWidget(QWidget *parent) : QWidget(parent)
{
    Initialize();
    m_nState = -1;
}

void ArrayValueVisitDrawingWidget::ConstructScalePart()
{
    ...
    m_pScalePart = _pPart;
    //m_pScalePart->setParent(this);
}

void ArrayValueVisitDrawingWidget::ResetAllParts()
{
...
void ArrayValueVisitDrawingWidget::Initialize()
{
    m_pService = nullptr;
    m_pScalePart = nullptr;
...
void ArrayValueVisitDrawingWidget::timerEvent(QTimerEvent *event)
{
...
void ArrayValueVisitDrawingWidget::AdjustInitialTransform()
{
...
void ArrayValueVisitDrawingWidget::ProcessNcShow(NNcShow::NcShow* pShow_)
{
...
QImage ArrayValueVisitDrawingWidget::GetImage()
{
    if(m_nState == 1)
    {
        return m_nImage;
...
void ArrayValueVisitDrawingWidget::paintEvent(QPaintEvent* event)
{
...
void ArrayValueVisitDrawingWidget::GetArrayParameter(
...
void ArrayValueVisitDrawingWidget::GetModalParameter(NFileParse::ArrayModalData& nModalData_)
{
...
void ArrayValueVisitDrawingWidget::Search4Clicked(bool checked_)
{
    if(m_nState == 1)
    {
        return;
...
void ArrayValueVisitDrawingWidget::Search7Clicked(bool checked_)
{
    if(m_nState == 1)
    {
        return;
    ...
    //
    m_pService->StartArray(
        NNcManager::NcManager::GetGeneralArraySearchFile((char*)"7"),
        _nArrNcParam,
        _nModalData,
        m_arrParts,
        m_nKeyAllocator,
    ...
    // OutPut All Part's Info For Debug
    // For Debug
    QString _filepath = NNcManager::NcManager::GetGeneralArraySearchFile((char*)"S7_show_test");
    QFile _file;
    _file.close();
    _file.setFileName(_filepath);
    bool _bRet = _file.open(
...
void ArrayValueVisitDrawingWidget::Generate4SearchFile()
{
    NParameter::ArrayNcParameter _nArrNcParam;
    GetArrayParameter(_nArrNcParam);
...
void ArrayValueVisitDrawingWidget::Generate7SearchFile()
{
    NParameter::ArrayNcParameter _nArrNcParam;
    GetArrayParameter(_nArrNcParam);
    NNcManager::NcManager::Instance()->GeneralArraySearchFileGenerate(
